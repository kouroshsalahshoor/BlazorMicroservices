...
namespace CartApi.Controllers
{
    ...
    public class CartsController : ControllerBase
    {
        ...
        public async Task<ResponseDto> GetCart(string userId)
        {
            try
            {
                var cartInDb = await _db.Carts.FirstAsync(x => x.UserId == userId);
        ...
        public async Task<ResponseDto> ApplyCoupon([FromBody] CartDto cartDto)
        {
            try
            {
                var cartFromDb = await _db.Carts.FirstAsync(x=>x.Id == cartDto.Id);
        ...
        public async Task<ResponseDto> CreateEdit([FromBody] CartDto cartDto)
        {
            try
            {
                var cartInDb = _db.Carts.AsNoTracking().FirstOrDefault(x => x.UserId == cartDto.UserId);
                if (cartInDb is null)
                {
                    //create cart
        ...
        public async Task<ResponseDto> Remove([FromBody] int cartDetailId)
        {
            try
            {
                ...
                if (count == 1)
                {
                    var cartInDb = await _db.Carts.FirstOrDefaultAsync(x => x.Id == cartDetailInDb.CartId);
                    _db.Carts.Remove(cartInDb);
                }
                await _db.SaveChangesAsync();
